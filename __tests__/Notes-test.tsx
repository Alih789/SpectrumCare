...
describe('NotesButton', () => {
    it('updates text input correctly', () => {
      const component = renderer.create(<NotesButton />);
      const instance = component.getInstance();
  
      const textInput = instance.root.findByProps({ testID: 'notes-text-input' });
      expect(textInput.props.value).toBe('Useless Multiline Placeholder');
  
      const newText = 'New text input value';
      instance.onChangeText(newText);
  
      expect(instance.state.text).toBe(newText);
      expect(textInput.props.value).toBe(newText);
    });
  });
