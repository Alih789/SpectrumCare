...
function PrepRoutePage({navigation, route}: Props): JSX.Element {
  ...
  useEffect(() => {
    async function getProcedureData() {
      const procedureDoc = await firestore()
      ...
      for (var page of allProcedureCollectionObjects.pages) {
        ...
        if (page.media) {
          if (page.media.contentType == 'image') {
            ...
            const videoID = page.media.content
              .trim()
              .slice(page.media.content.indexOf('v=') + 2);
            pagesDataAppReadyMediaContent.push({
              header: page.header,
              media: {
                content: videoID,
      ...
      setPagesData({
        title: allProcedureCollectionObjects.title,
