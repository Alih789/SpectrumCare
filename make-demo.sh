...
#######################################################################################################
# This script is from https://github.com/mikehardy/rnfbdemo, with the name of the app modified from
# 'rnfmdemo' to 'SpectrumCare'
# It was used to generate this project.

# it is included here as a reference for how the project was initialized and how firebase was added.
...
\rm -fr "$HOME/Library/Developer/Xcode/DerivedData/SpectrumCare*"
...
npm_config_yes=true npx @react-native-community/cli init SpectrumCare --skip-install --version=0.71.4
cd SpectrumCare
...
sed -i -e $'s/AppDelegate.h"/AppDelegate.h"\\\n#import <Firebase.h>/' ios/SpectrumCare/AppDelegate.m*
rm -f ios/SpectrumCare/AppDelegate.m*-e
sed -i -e $'s/self.moduleName/[FIRApp configure];\\\n  self.moduleName/' ios/SpectrumCare/AppDelegate.m*
rm -f ios/SpectrumCare/AppDelegate.m*-e
...
echo "For this demo to work, you must create an \`SpectrumCare\` project in your firebase console,"
...
if [ "$(uname)" == "Darwin" ]; then
  if [ -f "../GoogleService-Info.plist" ]; then
    cp ../GoogleService-Info.plist ios/SpectrumCare/
  else
...
if [ "$(uname)" == "Darwin" ]; then
  ...
  # set PRODUCT_BUNDLE_IDENTIFIER to com.SpectrumCare
  sed -i -e $'s/org.reactjs.native.example/com/' ios/SpectrumCare.xcodeproj/project.pbxproj
  rm -f ios/SpectrumCare.xcodeproj/project.pbxproj-e

  # Add our Google Services file to the Xcode project
  pbxproj file ios/SpectrumCare.xcodeproj SpectrumCare/GoogleService-Info.plist --target SpectrumCare

  # Toggle on iPad: add build flag: TARGETED_DEVICE_FAMILY = "1,2"
  pbxproj flag ios/SpectrumCare.xcodeproj --target SpectrumCare TARGETED_DEVICE_FAMILY "1,2"
fi
##################################################################################################
...
# sed -i -e $'s/  target \'SpectrumCareTests\' do/  $FirebaseSDKVersion = \'10.7.0\'\\\n  target \'SpectrumCareTests\' do/' ios/Podfile
# rm -f ios/Podfile??
...
#sed -i -e $'s/  target \'SpectrumCareTests\' do/  $FirebaseFirestoreExcludeLeveldb = true\\\n  pod \'FirebaseFirestore\', :git => \'https:\\/\\/github.com\\/invertase\\/firestore-ios-sdk-frameworks.git\', :tag => $FirebaseSDKVersion\\\n  target \'SpectrumCareTests\' do/' ios/Podfile
#rm -f ios/Podfile??
...
if [ "$(uname)" == "Darwin" ]; then
  ...
  #   # add file SpectrumCare/SpectrumCare.entitlements, with reference to SpectrumCare target, but no build phase
  #   echo "Adding macCatalyst entitlements file / build flags to Xcode project"
  #   cp ../SpectrumCare.entitlements ios/SpectrumCare/
  #   pbxproj file ios/SpectrumCare.xcodeproj SpectrumCare/SpectrumCare.entitlements --target SpectrumCare -C
  #   # add build flag: CODE_SIGN_ENTITLEMENTS = SpectrumCare/SpectrumCare.entitlements
  #   pbxproj flag ios/SpectrumCare.xcodeproj --target SpectrumCare CODE_SIGN_ENTITLEMENTS SpectrumCare/SpectrumCare.entitlements
  #   # add build flag: SUPPORTS_MACCATALYST = YES
  #   pbxproj flag ios/SpectrumCare.xcodeproj --target SpectrumCare SUPPORTS_MACCATALYST YES
  #   # add build flag 				DEVELOPMENT_TEAM = 2W4T2B656C;
  #   pbxproj flag ios/SpectrumCare.xcodeproj --target SpectrumCare DEVELOPMENT_TEAM "$XCODE_DEVELOPMENT_TEAM"
  #   #################################################################################################
  ...
  #   CATALYST_DESTINATION=$(xcodebuild -workspace ios/SpectrumCare.xcworkspace -configuration Debug -scheme SpectrumCare -destination id=7153382A-C92B-5798-BEA3-D82D195F25F8 2>&1|grep macOS|grep Catalyst|head -1 |cut -d':' -f5 |cut -d' ' -f1)
...
#   \rm -fr node_modules
#   echo "To run the app use Windows Powershell in the SpectrumCare directory with these commands:"
#   echo "npm i"
